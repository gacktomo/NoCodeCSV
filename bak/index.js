...
;(async () => {
  const targetColumnsPB = [
    {
      column: 'f',
      action: (str) =>
        str.match(/1200\d{11}/) ? str.match(/1200\d{11}/)[0] : '',
    },
    {
      column: 'p',
      action: (str) => `PB ${str}`,
  ...
  const pbFiles = (await readdirAsync('PB'))
    .filter((file) => file.match(/csv$/))
    .map((file) => `PB/${file}`)
  const ppFiles = (await readdirAsync('PP'))
    .filter((file) => file.match(/csv$/))
    .map((file) => `PP/${file}`)
  const tasks = [
    ...pbFiles.map((file) => loadCSV(file, targetColumnsPB)),
    ...ppFiles.map((file) => loadCSV(file, targetColumnsPP)),
  ]
